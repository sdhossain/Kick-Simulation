...
def dynamics(x, femoris, lateralis, medialis, intermedius,
    ...
    lt_femoris = femoris.norm_tendon_length(quad_muscle_length(x[0]), x[2])
    lt_lateralis = lateralis.norm_tendon_length(quad_muscle_length(x[0]), x[3])
    lt_medialis = medialis.norm_tendon_length(quad_muscle_length(x[0]), x[4])
    lt_intermedius = intermedius.norm_tendon_length(quad_muscle_length(x[0]), x[5])
    ...
    torque_gravity = gravity_moment(x[0])
    ...
    x_dot = np.array([0, 0, 0, 0, 0, 0, 0])

    x_dot[0] = x[1]
    x_dot[1] = (torque_quad + torque_gravity)/KNEE_INERTIA
    x_dot[2] = get_velocity(femoris_activation, x[2], lt_femoris, 
                    force_length_regression, force_velocity_regression)
    x_dot[3] = get_velocity(lateralis_activation, x[3], lt_lateralis, 
                    force_length_regression, force_velocity_regression)
    x_dot[4] = get_velocity(medialis_activation, x[4], lt_medialis, 
                    force_length_regression, force_velocity_regression)
    x_dot[5] = get_velocity(intermedius_activation, x[5], lt_intermedius, 
                    force_length_regression, force_velocity_regression)
